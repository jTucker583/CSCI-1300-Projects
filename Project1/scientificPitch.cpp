...
/* Works by using an outer do while loop, and resetting the menu_choice variable every time
a program is run, so that the user is re-directed to the main menu. Each program has input
validation that is done also by using do while loops. Basically, until the input is
checked against isValidTune() and returns true, the input validation loop keeps running.
After that, the specified program runs.
*/
int main()
{
    // The following variables can be reused in each of the program statements.
    string first_string;
    ...
    string menu_choice_string; // for getline statement, as it cannot accept int values.
    int menu_choice;           // what we are actually going to use.
    do
    {
        cout << "--- Menu ---\n"
        ...
        menu_choice = stoi(menu_choice_string); // converts menu_choice_string value into an int to be stored.
        // cout << menu_choice << endl;
        if (menu_choice == 1) // tuneSimilarity()
        {
            ...
            do
            {
                ...
                if (isValidTune(first_string) == false)
                {
                    cout << "Invalid input. Please enter a tune in valid SPN: " << endl;
                }
            } while (isValidTune(first_string) == false);
            // cout << "First String: " << first_string << endl;
            cout << "Please enter the second tune:" << endl;
            do
            {
                getline(cin, second_string);
                if (isValidTune(second_string) == false)
                {
                    cout << "Invalid input. Please enter a tune in valid SPN: " << endl;
                }
            } while (isValidTune(second_string) == false);
            // cout << "Second String: " << second_string << endl;
            similarity_score = tuneSimilarity(first_string, second_string);
            cout << "The similarity score is " << similarity_score << endl;
            menu_choice = 0;
        }

        else if (menu_choice == 2) // bestSimilarity()
        {
            ...
            do
            {
                ...
                if (isValidTune(target_string) == false)
                {
                    cout << "Invalid input. Please enter a tune in valid SPN: " << endl;
                }
            } while (isValidTune(target_string) == false);
            similarity_score = bestSimilarity(first_string, target_string);
            cout << setprecision(2) << fixed;
            cout << "The best similarity score is: " << similarity_score << endl;
            menu_choice = 0;
        }

        else if (menu_choice == 3) // printTuneRanking()
        {
            cout << "Please enter the first tune:" << endl;
            do
            {
            ...
            do
            {
                getline(cin, third_string);
                if (isValidTune(third_string) == false)
                {
                    cout << "Invalid input. Please enter a tune in valid SPN: " << endl;
                }
            } while (isValidTune(third_string) == false);
            ...
            printTuneRankings(first_string, second_string, third_string, target_string);
            menu_choice = 0;
        }

        else if (menu_choice == 4) // exit program
        {
            cout << "Goodbye!" << endl;
            return 0;
        }
        else // input validation for main menu loop
        {
